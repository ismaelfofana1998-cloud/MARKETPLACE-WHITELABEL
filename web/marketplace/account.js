...
async function rendreAuthentification() {
  ...
  const afficher = (mode) => {
    ...
    document.querySelector("#auth-form").addEventListener("submit", async (event) => {
      ...
      if (!resultat.data.session) return toast("Compte créé. Confirme ton email pour te connecter.");
...
function carteCommande(achat) {
  ...
  return `<article class="carte"><div class="ligne-entre"><div><p class="muted petit">${formatDate(achat.cree_le)} - ${commandes.length} boutique${commandes.length > 1 ? "s" : ""}</p><h3>${escapeHtml(achat.reference)}</h3></div>${badgeStatut(statut)}</div><div class="ligne-entre" style="margin-top:14px"><div><strong>${fcfa(achat.total)}${fraisAConfirmer ? " + livraison" : ""}</strong><p class="muted petit" style="margin:3px 0 0">${fraisAConfirmer ? "Frais confirmés lors de la prise en charge" : achat.mode_paiement === "A_LA_LIVRAISON" ? "Paiement à la livraison" : escapeHtml(achat.mode_paiement)}</p></div><button class="btn btn-secondaire" data-detail-achat="${achat.id}">${icone("eye")} Détails</button></div></article>`;
}
...
export async function rendreCompte() {
  ...
  try {
    ...
    coquille(`<main class="conteneur"><div class="entete-page"><div><h1>Bonjour ${escapeHtml(prenom)}</h1><p class="muted">${escapeHtml(etat.session.user.email)}</p></div><button class="btn btn-secondaire" id="deconnexion">${icone("log-out")} Déconnexion</button></div><div class="onglets" id="compte-tabs"><button class="onglet actif" data-tab="commandes">Commandes</button><button class="onglet" data-tab="profil">Profil</button><button class="onglet" data-tab="adresses">Adresses</button><button class="onglet" data-tab="favoris">Favoris</button><button class="onglet" data-tab="pro">Espace pro</button>${donnees.administrateur ? '<a class="onglet" href="./admin.html">Administration</a>' : ""}</div><section class="section" id="compte-zone"></section></main><dialog class="dialogue" id="detail-dialog"><div class="dialogue-entete"><h2>Détail de la commande</h2><button class="dialogue-fermer" data-fermer aria-label="Fermer">${icone("x")}</button></div><div class="dialogue-corps" id="detail-zone"></div></dialog>`, { actif: "compte" });
    const zone = document.querySelector("#compte-zone");
    const afficherCommandes = () => {
      zone.innerHTML = donnees.achats.length ? `<div class="pile">${donnees.achats.map(carteCommande).join("")}</div>` : vide("package-open", "Aucune commande", "Tes prochaines commandes apparaîtront ici.", '<a class="btn btn-primaire" href="./index.html">Découvrir le catalogue</a>');
    ...
    const ouvrirDetail = (achatId) => {
      ...
      document.querySelectorAll("[data-annuler]").forEach((button) => button.addEventListener("click", async () => {
        ...
        toast("Commande annulée et stock restitué");
        location.reload();
      }));
    ...
    const afficherProfil = () => {
      zone.innerHTML = `<form class="carte" id="profil-form"><h2>Mes informations</h2><div class="grille-deux"><div class="champ"><label>Prénom</label><input name="prenom" value="${escapeHtml(donnees.profil.prenom)}" autocomplete="given-name"></div><div class="champ"><label>Nom</label><input name="nom" value="${escapeHtml(donnees.profil.nom)}" autocomplete="family-name"></div></div><div class="champ"><label>Téléphone</label><input name="telephone" type="tel" value="${escapeHtml(donnees.profil.telephone)}" autocomplete="tel"></div><div class="champ"><label>Zone de livraison habituelle</label>${zones.length ? `<select name="zone_livraison"><option value="">Non renseignée</option>${optionsZones(donnees.profil.zone_livraison || "")}</select>` : `<input name="zone_livraison" value="${escapeHtml(donnees.profil.zone_livraison)}" placeholder="COCODY">`}</div><button class="btn btn-primaire" id="enregistrer-profil">${icone("save")} Enregistrer</button></form>`;
    ...
    const afficherPro = () => {
      ...
      zone.innerHTML = `<div class="grille-deux">${donnees.administrateur ? `<article class="carte"><h2>Administration de la plateforme</h2><p class="muted">Pilotage des tenants, boutiques, commandes, livraisons et de l'apparence.</p><a class="btn btn-primaire" href="./admin.html">${icone("shield-check")} Ouvrir l'administration</a></article>` : ""}<article class="carte"><h2>Espace marchand</h2><p class="muted">${marchandes.length ? `Tu as accès à ${marchandes.length} organisation${marchandes.length > 1 ? "s" : ""} marchande${marchandes.length > 1 ? "s" : ""}.` : "Crée ta boutique, ajoute tes articles et traite les commandes de ton équipe."}</p><a class="btn btn-primaire" href="./marchand.html">${icone("store")} ${marchandes.length ? "Ouvrir l'espace marchand" : "Créer ma boutique"}</a></article><article class="carte"><h2>Identité et équipe</h2><p class="muted">Gère tes organisations, tes salariés et leurs rôles depuis IKIGAI Identity.</p><a class="btn btn-secondaire" href="../identity/index.html">${icone("users")} Gérer mes organisations</a></article></div>`;
      rafraichirIcones(zone);
    };
