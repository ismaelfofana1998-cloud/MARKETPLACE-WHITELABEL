...
export async function rendreCompte() {
  ...
  try {
    ...
    const ouvrirDetail = (achatId) => {
      ...
      const commandes = achat.commandes_marketplace || [];
      document.querySelector("#detail-zone").innerHTML = `<div class="ligne-entre"><div><p class="muted petit">${formatDate(achat.cree_le, true)}</p><h3>${escapeHtml(achat.reference)}</h3></div><strong>${fcfa(achat.total)}</strong></div><hr class="separateur"><div class="pile">${commandes.map((commande) => `<section class="carte"><div class="ligne-entre"><div><p class="muted petit">${escapeHtml(commande.boutiques?.nom || "Boutique")}</p><strong>${escapeHtml(commande.reference)}</strong></div>${badgeStatut(commande.statut)}</div><div class="pile" style="margin-top:13px">${(commande.lignes_commande_marketplace || []).map((ligne) => `<div class="ligne"><img src="${escapeHtml(ligne.image_url || etat.configuration.hero_image_url)}" alt="" style="width:48px;height:48px;object-fit:cover;border-radius:5px"><div style="flex:1"><strong class="petit">${escapeHtml(ligne.nom_produit)}</strong><p class="muted petit" style="margin:3px 0">${ligne.quantite} x ${fcfa(ligne.prix_unitaire)}</p></div></div>`).join("")}</div><div class="ligne-entre" style="margin-top:13px"><strong>${fcfa(commande.total)}</strong>${["NOUVELLE", "CONFIRMEE"].includes(commande.statut) ? `<button class="btn btn-danger" data-annuler="${commande.id}">${icone("x-circle")} Annuler</button>` : ""}</div>${commande.motif_annulation ? `<p class="petit muted" style="margin:10px 0 0">Motif : ${escapeHtml(commande.motif_annulation)}</p>` : ""}</section>`).join("")}</div><hr class="separateur"><div><strong>Livraison</strong><p class="muted petit" style="margin:5px 0 0">${escapeHtml(achat.adresses_livraison?.destinataire_nom || "")} - ${escapeHtml(achat.adresses_livraison?.telephone || "")}<br>${escapeHtml(achat.adresses_livraison?.adresse || "")} ${escapeHtml(achat.adresses_livraison?.commune || "")}</p></div>`;
