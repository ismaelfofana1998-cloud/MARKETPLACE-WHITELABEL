...
function ouvrirProduit(donnees, produit) {
  ...
  document.querySelector("#dialog-zone").innerHTML = `<form id="produit-form"><div class="champ"><label>Nom</label><input name="nom" value="${escapeHtml(produit?.nom)}" required></div><div class="grille-deux"><div class="champ"><label>Marque</label><input name="marque" value="${escapeHtml(produit?.marque)}"></div><div class="champ"><label>Categorie</label><select name="categorie_id"><option value="">Sans categorie</option>${donnees.categories.map((categorie) => `<option value="${categorie.id}" ${categorie.id === produit?.categorie_id ? "selected" : ""}>${escapeHtml(categorie.nom)}</option>`).join("")}</select></div></div><div class="champ"><label>Description</label><textarea name="description">${escapeHtml(produit?.description)}</textarea></div><div class="grille-deux"><div class="champ"><label>Prix (FCFA)</label><input name="prix" type="number" min="0" step="1" value="${produit?.prix ?? ""}" required></div><div class="champ"><label>Stock</label><input name="stock" type="number" min="0" step="1" value="${stock}" required></div></div><div class="champ"><label>Statut</label><select name="statut"><option value="ACTIF" ${produit?.statut === "ACTIF" ? "selected" : ""}>Actif</option><option value="BROUILLON" ${produit?.statut === "BROUILLON" ? "selected" : ""}>Brouillon</option><option value="EPUISE" ${produit?.statut === "EPUISE" ? "selected" : ""}>Epuise</option><option value="ARCHIVE" ${produit?.statut === "ARCHIVE" ? "selected" : ""}>Archive</option></select></div><div class="champ"><label>Photos</label><input name="photos" type="file" accept="image/jpeg,image/png,image/webp" multiple><span class="champ-aide">Jusqu'a 5 Mo par image. Les nouvelles photos remplacent les anciennes.</span></div>${produit?.images?.length ? `<div class="ligne" style="overflow-x:auto">${produit.images.map((image) => `<img src="${escapeHtml(image)}" alt="" style="width:70px;height:70px;object-fit:cover;border-radius:6px">`).join("")}</div>` : ""}<button class="btn btn-primaire btn-bloc" id="sauver-produit" style="margin-top:17px">${icone("save")} Enregistrer le produit</button></form>`;
  document.querySelector("#produit-form").addEventListener("submit", async (event) => {
    ...
    try {
      const fichiers = [...form.elements.photos.files].slice(0, 6);
...
function afficherBoutique(donnees) {
  ...
  zone.innerHTML = `<form class="carte" id="boutique-form"><h2>Personnaliser la boutique</h2><div class="champ"><label>Nom</label><input name="nom" value="${escapeHtml(boutique.nom)}" required></div><div class="champ"><label>Description</label><textarea name="description">${escapeHtml(boutique.description)}</textarea></div><div class="grille-deux"><div class="champ"><label>Telephone</label><input name="telephone" type="tel" value="${escapeHtml(boutique.telephone)}"></div><div class="champ"><label>WhatsApp</label><input name="whatsapp" type="tel" value="${escapeHtml(boutique.whatsapp)}"></div></div><div class="champ"><label>Email de contact</label><input name="email_contact" type="email" value="${escapeHtml(boutique.email_contact)}"></div><div class="champ"><label>Adresse</label><input name="adresse" value="${escapeHtml(boutique.adresse)}"></div><div class="grille-deux"><div class="champ"><label>Frais de livraison de base</label><input name="frais_livraison_base" type="number" min="0" value="${boutique.frais_livraison_base}"></div><div class="champ"><label>Delai de preparation (minutes)</label><input name="delai_preparation_minutes" type="number" min="0" value="${boutique.delai_preparation_minutes || 60}"></div></div><div class="champ"><label>Publication</label><select name="statut"><option value="BROUILLON" ${boutique.statut === "BROUILLON" ? "selected" : ""}>Brouillon</option><option value="PUBLIEE" ${boutique.statut === "PUBLIEE" ? "selected" : ""}>Publiee</option></select></div><div class="grille-deux"><div class="champ"><label>Logo</label><input name="logo" type="file" accept="image/jpeg,image/png,image/webp">${boutique.logo_url ? `<img src="${escapeHtml(boutique.logo_url)}" alt="" style="width:90px;height:90px;object-fit:cover;border-radius:6px">` : ""}</div><div class="champ"><label>Banniere</label><input name="banniere" type="file" accept="image/jpeg,image/png,image/webp">${boutique.banniere_url ? `<img src="${escapeHtml(boutique.banniere_url)}" alt="" style="width:160px;height:90px;object-fit:cover;border-radius:6px">` : ""}</div></div><button class="btn btn-primaire" id="sauver-boutique">${icone("save")} Enregistrer</button></form>`;
