...
async function rendreAuth(modeInitial = "connexion") {
  ...
  const optionsZones = catalogueZones.zones
    ...
    .join("");
  app.innerHTML = `<div class="auth-shell"><section class="auth-visuel"><a class="marque" href="../marketplace/index.html"><span style="color:white">IKIGAI</span> <span>Identity</span></a><div><h1>Un compte pour tout l'écosystème IKIGAI.</h1><p>Marketplace, livraison et services professionnels partagent votre identité, sans mélanger les données métier.</p></div><p class="auth-bas petit">IKIGAI Software</p></section><section class="auth-panneau"><div class="auth-form">${new URLSearchParams(location.search).has("invitation") ? '<div class="bande-info" style="margin-bottom:17px"><strong>Invitation reçue</strong><p class="petit" style="margin:5px 0 0">Connecte-toi ou crée ton compte avec l\'email invité pour rejoindre l\'organisation.</p></div>' : ""}<div class="onglets"><button class="onglet" data-auth="connexion">Connexion</button><button class="onglet" data-auth="inscription">Créer un compte</button></div><div id="auth-zone" style="padding-top:21px"></div></div></section></div>`;
  const afficher = (mode) => {
    const inscription = mode === "inscription";
    document.querySelectorAll("[data-auth]").forEach((button) => button.classList.toggle("actif", button.dataset.auth === mode));
    document.querySelector("#auth-zone").innerHTML = `<form id="auth-form">${inscription ? `<div class="grille-deux"><div class="champ"><label>Prénom</label><input name="prenom" required></div><div class="champ"><label>Nom</label><input name="nom" required></div></div><div class="champ"><label>Téléphone</label><input name="telephone" type="tel"></div><div class="champ"><label>Zone de livraison habituelle</label>${optionsZones ? `<select name="zone_livraison" required><option value="">Choisir une zone</option>${optionsZones}</select>` : '<input name="zone_livraison" placeholder="COCODY" required>'}<span class="champ-aide">Cette zone sera modifiable pour chaque commande.</span></div>` : ""}<div class="champ"><label>Email</label><input name="email" type="email" autocomplete="email" required></div><div class="champ"><label>Mot de passe</label><input name="password" type="password" minlength="8" autocomplete="${inscription ? "new-password" : "current-password"}" required></div><button class="btn btn-primaire btn-bloc" id="auth-submit">${icone(inscription ? "user-plus" : "log-in")} ${inscription ? "Créer mon compte" : "Se connecter"}</button></form>${!inscription ? '<button class="btn btn-texte" id="oublie" style="margin-top:10px">Mot de passe oublié</button>' : ""}`;
    document.querySelector("#auth-form").addEventListener("submit", async (event) => {
      ...
      if (!resultat.data.session) return toast("Compte créé. Confirme ton email avant de revenir sur ce lien.");
      await chargerDonnees();
    ...
    document.querySelector("#oublie")?.addEventListener("click", async () => {
      ...
      resetErreur ? erreur(resetErreur) : toast("Lien de réinitialisation envoyé");
    });
...
function rendreDashboard() {
  app.innerHTML = `<div class="app"><header class="bandeau"><div class="bandeau-ligne"><a class="marque" href="../marketplace/index.html"><span style="color:white">IKIGAI</span> <span>Identity</span></a><span class="bandeau-espace">Compte et accès</span><div class="bandeau-actions"><a class="icone-btn" href="../marketplace/index.html" title="Marketplace">${icone("store")}</a><button class="icone-btn" id="deconnexion" title="Déconnexion">${icone("log-out")}</button></div></div></header><div class="conteneur mise-en-page"><nav class="menu-lateral"><button data-tab="profil">${icone("user-round")} Mon profil</button><button data-tab="organisations">${icone("building-2")} Organisations</button><button data-tab="equipe">${icone("users")} Équipe</button><button data-tab="securite">${icone("shield-check")} Sécurité</button></nav><section id="identity-zone"></section></div><nav class="bottom-nav"><button data-tab="profil">${icone("user-round")}<span>Profil</span></button><button data-tab="organisations">${icone("building-2")}<span>Organisations</span></button><button data-tab="equipe">${icone("users")}<span>Équipe</span></button><a href="../marketplace/index.html">${icone("store")}<span>Market</span></a></nav></div><dialog class="dialogue" id="identity-dialog"><div class="dialogue-entete"><h2 id="dialog-title"></h2><button class="dialogue-fermer" data-fermer>${icone("x")}</button></div><div class="dialogue-corps" id="dialog-zone"></div></dialog>`;
...
async function afficherProfil() {
  ...
  zone.innerHTML = `<div class="entete-page"><div><h1>Mon profil</h1><p class="muted">${escapeHtml(etat.session.user.email)}</p></div></div><form class="carte" id="profil-form"><div class="grille-deux"><div class="champ"><label>Prénom</label><input name="prenom" value="${escapeHtml(etat.identite.prenom)}"></div><div class="champ"><label>Nom</label><input name="nom" value="${escapeHtml(etat.identite.nom)}"></div></div><div class="champ"><label>Téléphone</label><input name="telephone" type="tel" value="${escapeHtml(etat.identite.telephone)}"></div><div class="champ"><label>Zone de livraison habituelle</label>${optionsZones ? `<select name="zone_livraison"><option value="">Non renseignée</option>${optionsZones}</select>` : `<input name="zone_livraison" value="${escapeHtml(etat.identite.zone_livraison)}" placeholder="COCODY">`}</div><div class="champ"><label>Langue</label><select name="langue"><option value="fr" ${etat.identite.langue === "fr" ? "selected" : ""}>Français</option><option value="en" ${etat.identite.langue === "en" ? "selected" : ""}>English</option></select></div><button class="btn btn-primaire">${icone("save")} Enregistrer</button></form>`;
...
function afficherOrganisations() {
  const zone = document.querySelector("#identity-zone");
  zone.innerHTML = `<div class="entete-page"><div><h1>Organisations</h1><p class="muted">Tes accès professionnels dans l'écosystème IKIGAI</p></div><button class="btn btn-primaire" id="nouvelle-organisation">${icone("plus")} Nouvelle</button></div><div class="pile">${etat.organisations.map((organisation) => `<article class="carte ligne-entre"><div><h3>${escapeHtml(organisation.nom)}</h3><div class="ligne"><span class="badge">${escapeHtml(organisation.type)}</span><span class="muted petit">${escapeHtml(organisation.role)}</span></div></div>${etat.active?.id === organisation.id ? '<span class="badge badge-succes">Active</span>' : `<button class="btn btn-secondaire" data-activer="${organisation.id}">Activer</button>`}</article>`).join("") || '<div class="vide">Aucune organisation</div>'}</div>`;
...
async function afficherEquipe() {
  const zone = document.querySelector("#identity-zone");
  if (!etat.active) {
    zone.innerHTML = '<div class="vide"><h2>Aucune organisation active</h2><p>Crée ou rejoins une organisation pour gérer une équipe.</p></div>';
    return;
  }
  zone.innerHTML = `<div class="entete-page"><div><h1>Équipe</h1><p class="muted">${escapeHtml(etat.active.nom)}</p></div>${["PROPRIETAIRE", "ADMIN"].includes(etat.active.role) ? `<button class="btn btn-primaire" id="inviter">${icone("user-plus")} Inviter</button>` : ""}</div><div id="membres-zone" class="vide">Chargement...</div><div id="invitation-resultat" style="margin-top:15px"></div>`;
  ...
  zone.querySelector("#membres-zone").outerHTML = `<div class="table-wrap"><table><thead><tr><th>Membre</th><th>Email</th><th>Rôle</th><th>Statut</th></tr></thead><tbody>${(data || []).map((membre) => `<tr><td><strong>${escapeHtml(`${membre.identites?.prenom || ""} ${membre.identites?.nom || ""}`.trim())}</strong></td><td>${escapeHtml(membre.identites?.email || "")}</td><td>${escapeHtml(membre.role)}</td><td><span class="badge">${escapeHtml(membre.statut)}</span></td></tr>`).join("")}</tbody></table></div>`;
  zone.querySelector("#inviter")?.addEventListener("click", () => {
    document.querySelector("#dialog-title").textContent = "Inviter un membre";
    document.querySelector("#dialog-zone").innerHTML = `<form id="invitation-form"><div class="champ"><label>Email</label><input name="email" type="email" required></div><div class="champ"><label>Rôle</label><select name="role"><option value="AGENT">Agent</option><option value="GESTIONNAIRE">Gestionnaire</option><option value="ADMIN">Administrateur</option><option value="MEMBRE">Membre</option></select></div><button class="btn btn-primaire btn-bloc">Créer l'invitation</button></form>`;
    document.querySelector("#invitation-form").addEventListener("submit", async (event) => {
      ...
      zone.querySelector("#invitation-resultat").innerHTML = `<div class="bande-info"><strong>Lien copié</strong><p class="petit" style="word-break:break-all;margin:6px 0 0">${escapeHtml(lien)}</p></div>`;
      toast("Invitation créée");
    });
    document.querySelector("#identity-dialog").showModal();
  });
  rafraichirIcones(zone);
